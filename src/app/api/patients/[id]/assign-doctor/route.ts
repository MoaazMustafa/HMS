  ...
  try {
    ...
    const assignment = await prisma.patientDoctorAssignment.create({
      data: {
        ...
        doctorId: doctorId,
        status: 'ACTIVE',
        notes: notes || `Added to care team by Dr. ${currentDoctor.firstName} ${currentDoctor.lastName}`,
      },
    });
    ...
    return NextResponse.json({ error: 'Failed to add doctor assignment' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to remove doctor assignment' }, { status: 500 });
  }
}
